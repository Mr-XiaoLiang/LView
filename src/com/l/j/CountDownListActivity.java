...
public class CountDownListActivity extends Activity {
	...
	private void init() {
		beans = new ArrayList<CountDownListActivity.Bean>();
		for (int i = 0; i < 50; i++) {
			CountDownListActivity.Bean bean = new Bean(System.currentTimeMillis(), "这是第"+(i+1)+"条，起始：" + i + ":00", (i*60),i+1);
			beans.add(bean);
		}
...
//			LCountDownView2 downView2;
	...
	private class GSCListener implements OnLGoodsSizeChangeViewListener{
		@Override
