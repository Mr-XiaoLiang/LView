...
import com.l.j.option.LAnnularMenuViewOption;
import com.l.j.option.LAnnularMenuViewOption.BackgroundType;
import com.l.j.option.LAnnularMenuViewOption.GravityType;
...
public class LAnnularMenuView extends View {
	...
	private void init() {
		...
		case Top:
			...
			if (option.getGravityType() == GravityType.Top) {
		...
		case Right:
			...
			itemDiameter = (int) (radius / option.getItemProportionWithRing());
			if (option.getGravityType() == GravityType.Left) {
		...
		if(option.getBg()!=null){
			if(option.getBackgroundType()==BackgroundType.S){
